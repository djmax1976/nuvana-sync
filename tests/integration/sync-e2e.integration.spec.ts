...
describe('Sync E2E Integration Tests', () => {
  ...
  // ==========================================================================
  // E2E-010: Complete Bidirectional Flow
  // ==========================================================================
  describe('E2E-010: Complete bidirectional employee sync flow', () => {
    it('should support local create -> push -> cloud ID assignment', async () => {
      const { syncQueueDAL } = await import('../../src/main/dal/sync-queue.dal');
      const { cloudApiService } = await import('../../src/main/services/cloud-api.service');
      const { usersDAL } = await import('../../src/main/dal/users.dal');

      // Step 1: Create local employee
      const localEmployee = await usersDAL.create({
        store_id: E2E_STORE_ID,
        role: 'cashier',
        name: 'Bidirectional Test Employee',
        pin: '9999',
      });

      // Step 2: Enqueue for cloud push
      syncQueueDAL.enqueue({
        store_id: E2E_STORE_ID,
        entity_type: 'employee',
        entity_id: localEmployee.user_id,
        operation: 'CREATE',
        payload: {
          user_id: localEmployee.user_id,
          store_id: localEmployee.store_id,
          cloud_user_id: null,
      ...
      const pushResult = await cloudApiService.pushEmployees([
        {
          user_id: localEmployee.user_id,
          store_id: localEmployee.store_id,
          cloud_user_id: null,
          role: localEmployee.role as 'cashier',
          name: localEmployee.name,
          active: true,
        },
      ]);
      ...
      expect(pushResult.results[0].status).toBe('synced');
      expect(pushResult.results[0].cloud_user_id).toBeDefined();

      // Step 4: Mark as synced
      syncQueueDAL.markSynced(enqueueCallHistory[0].entity_id);
      expect(syncQueueDAL.markSynced).toHaveBeenCalled();
    });
  });
});
