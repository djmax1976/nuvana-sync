...
describe('NAXML Movement Report Type Definitions Unit Tests', () => {
  ...
  describe('Type Definition Tests', () => {
    ...
    it('MR-TYPE-004: Movement report type should only allow valid types', () => {
      expect(NAXMLMovementReportTypeSchema.parse('FuelGradeMovement')).toBe('FuelGradeMovement');
      expect(NAXMLMovementReportTypeSchema.parse('FuelProductMovement')).toBe(
        'FuelProductMovement'
      );
      expect(NAXMLMovementReportTypeSchema.parse('MiscellaneousSummaryMovement')).toBe(
        'MiscellaneousSummaryMovement'
      );
      expect(NAXMLMovementReportTypeSchema.parse('TaxLevelMovement')).toBe('TaxLevelMovement');
      expect(NAXMLMovementReportTypeSchema.parse('MerchandiseCodeMovement')).toBe(
        'MerchandiseCodeMovement'
      );
      expect(NAXMLMovementReportTypeSchema.parse('ItemSalesMovement')).toBe('ItemSalesMovement');
      expect(NAXMLMovementReportTypeSchema.parse('TankProductMovement')).toBe(
        'TankProductMovement'
      );
      expect(() => NAXMLMovementReportTypeSchema.parse('InvalidType')).toThrow();
    });
  ...
  describe('Movement Header Schema Tests', () => {
    ...
    it('MR-SCHEMA-010: should validate sales movement header', () => {
      const result = NAXMLSalesMovementHeaderSchema.safeParse(VALID_SALES_MOVEMENT_HEADER);
      expect(result.success).toBe(true);
    });
  });
  ...
  describe('FGM (Fuel Grade Movement) Schema Tests', () => {
    ...
    it('MR-FGM-003: should validate FGM sales totals', () => {
      const result = NAXMLFGMSalesTotalsSchema.safeParse(VALID_FGM_SALES_TOTALS);
      expect(result.success).toBe(true);
    });
    ...
    it('MR-FGM-008: should validate FGM detail with position summary', () => {
      const result = NAXMLFGMDetailSchema.safeParse(VALID_FGM_DETAIL_BY_POSITION);
      expect(result.success).toBe(true);
    });
    ...
    it('MR-FGM-018: should validate FGM with non-resettable totals', () => {
      const result = NAXMLFGMDetailSchema.safeParse(VALID_FGM_DETAIL_BY_POSITION);
      expect(result.success).toBe(true);
      if (result.success && result.data.fgmPositionSummary?.fgmNonResettableTotal) {
        expect(
          result.data.fgmPositionSummary.fgmNonResettableTotal.fuelGradeNonResettableTotalVolume
        ).toBe(186261.24);
      }
    });
  ...
  describe('FPM (Fuel Product Movement) Schema Tests', () => {
    ...
    it('MR-FPM-002: should validate complete FPM data', () => {
      const result = NAXMLFuelProductMovementDataSchema.safeParse(VALID_FPM_DATA);
      expect(result.success).toBe(true);
    });
  ...
  describe('MSM (Miscellaneous Summary Movement) Schema Tests', () => {
    ...
    it('MR-MSM-002: should validate complete MSM data', () => {
      const result = NAXMLMiscellaneousSummaryMovementDataSchema.safeParse(VALID_MSM_DATA);
      expect(result.success).toBe(true);
    });
    ...
    it('MR-MSM-004: safeValidateMiscellaneousSummaryMovementData should return success', () => {
      const result = safeValidateMiscellaneousSummaryMovementData(VALID_MSM_DATA);
      expect(result.success).toBe(true);
    });
  ...
  describe('MCM (Merchandise Code Movement) Schema Tests', () => {
    ...
    it('MR-MCM-002: should validate complete MCM data', () => {
      const result = NAXMLMerchandiseCodeMovementDataSchema.safeParse(VALID_MCM_DATA);
      expect(result.success).toBe(true);
    });
  ...
  describe('Transmission Header Schema Tests', () => {
    it('should validate transmission header', () => {
      const result = NAXMLTransmissionHeaderSchema.safeParse(VALID_TRANSMISSION_HEADER);
      expect(result.success).toBe(true);
    });
