...
export function BinSelector({
  ...
  // Find selected bin
  const selectedBin = useMemo(() => bins.find((b) => b.bin_id === value) || null, [bins, value]);
    ...
    <div className="space-y-2">
      ...
      <Select value={value} onValueChange={handleChange} disabled={disabled}>
        ...
        <SelectContent data-testid={testId ? `${testId}-dropdown` : undefined}>
          {bins.length === 0 ? (
            <div className="p-2 text-center text-sm text-muted-foreground">No bins configured</div>
          ) : (
            bins.map((bin) => {
              ...
              return (
                <SelectItem
                  key={bin.bin_id}
                  value={bin.bin_id}
                  data-testid={testId ? `${testId}-option-${bin.bin_id}` : undefined}
                >
      ...
      {isOccupied && selectedBin && (
        <Alert className="mt-2" data-testid="occupied-bin-warning">
          ...
          <AlertDescription>
            ...
            <strong>{selectedBin.pack?.game_name}</strong> (Pack #{selectedBin.pack?.pack_number}).
            It will be marked as sold.
          </AlertDescription>
        </Alert>
      )}
    </div>
